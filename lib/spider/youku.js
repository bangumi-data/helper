...
module.exports = async (rawId) => {
  ...
  let info = {};
  try {
    const { vid, begin = '' } = await getVid(id);
    const { official = false } = await getOfficial(vid);
    const { premuiumOnly = false } = await getPremuiumOnly(id);
    info = { begin, official, premuiumOnly };
  } catch (e) {
    console.log(e);
  }
