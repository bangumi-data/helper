 ...
 * @param  {Function} options.matchBegin 匹配番剧播放日的方法
 * @param  {boolean} options.mergeTitleTranslate 是否合并标题翻译 默认合并
 * @param  {boolean} options.matchMyanimelistId 是否只用 MyAnimeListId 匹配
...
exports.matchBangumi = async function matchBangumi(input, options) {
  ...
  for (const item of options.items) {
    ...
    // match with myanimelistid
    if (options.matchMyanimelistId === true) {
      fileToAdd = existingBangumi.map((localFile) => {
        let titleMatch = localFile.result.map((existingBangumi) => {
          return existingBangumi.sites.some((site) => site.site === 'mal' && site.id === item.myanimelistid) 
            ? existingBangumi.title
            : null;
      ...
      fileToAdd = existingBangumi.map((localFile) => {
        let titleMatch = localFile.result.map((existingBangumi) => {
          ...
          if (matchedTitle) {
            // check begin date
            if (options.matchBegin) {
              if (!options.matchBegin(existingBangumi.begin, item)) {
                matchedTitle = null;
              }
            }
          }
    ...
    if (fileToAdd.length === 1) {
      ...
      if (options.mergeTitleTranslate === false) {
        objectForMerge.titleTranslate = undefined;
      }
