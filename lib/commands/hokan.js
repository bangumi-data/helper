...
async function getExclusions(site) {
  ...
  const api = `https://raw.githubusercontent.com/bangumi-data/helper/master/exclusions/${site}.txt`;
  const remote = await fetch(api).then(res => (res.status === 404 ? '' : res.text()));
...
module.exports = async function hokan({ site, input, output }) {
  ...
  const todos = items
    ...
    .map(item => Object.assign({ copyIt: { site, id: item.id } }, item));
