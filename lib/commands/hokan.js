...
async function housekeepExclusions(site, items, exclusions) {
  const spinner = ora('Housekeeping exclusions').start();
  const itemIds = items.map((item) => item.id);
  const validExclusions = exclusions.filter((id) => itemIds.includes(id));
  const exclusionFile = path.resolve(__dirname, `../../exclusions/${site}.txt`);
  await fs.outputFile(exclusionFile, validExclusions.join('\n'));
  spinner.stop();
}
...
module.exports = async function hokan({ site, input, output }) {
  ...
  await housekeepExclusions(site, items, exclusions);
