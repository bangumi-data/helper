...
exports.getBegin = async function getBegin(id) {
  ...
  const url = `https://disney.content.edge.bamgrid.com/svc/content/DmcSeriesBundle/version/${API_VERSION}/region/${REGION}audience/k-false,l-false/maturity/9999/language/zh-Hant/encodedSeriesId/${id}`;
  const json = await fetch(url).then((res) => res.json());
...
exports.getIsBangumiOffline = async (id) => {
  const url = `https://disney.content.edge.bamgrid.com/svc/content/DmcSeriesMeta/version/${API_VERSION}/region/${REGION}/audience/k-false,l-false/maturity/9999/language/zh-Hant/encodedSeriesId/${id}`;
  const json = await fetch(url).then((res) => res.json());
  const metadata = json.data.DmcSeriesMeta;
  return metadata.series === null && metadata.season === null && metadata.episode === null;
}
