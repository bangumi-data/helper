...
async function getAllByLang(lang = 'en', page, titleOnly = false) {
  // fetch bangumi
  const url = `https://disney.content.edge.bamgrid.com/svc/content/CuratedSet/version/6.0/region/TW/audience/k-false,l-false/maturity/9999/language/${lang}/setId/${setId}/pageSize/60/page/${page}`;
...
exports.getAll = async function getAll() {
  ...
  const brandUrl = 'https://disney.content.edge.bamgrid.com/svc/content/Collection/StandardCollection/version/6.0/region/HK/audience/k-false,l-true/maturity/9999/language/en-GB/contentClass/brand/slug/star';
  const brandJson = await fetch(brandUrl).then((res) => res.json());
  const set = brandJson.data.Collection.containers.find((item) => item.set.text.title.full.set.default.content === 'Japanese Anime');
  setId = set.set.refId;
