...
exports.getAllBangumi = async function getAllBangumi(region, page, totalPage) {
  ...
  // crawl pre-air bangumi with seasonal page
  return items.concat(await getAllBangumiSeasonal(region, headers));
}

async function getAllBangumiSeasonal(region, headers) {
  const date = new Date();
  const year = date.getFullYear();
  const month = date.getMonth() + 1;
  let season;
  if (month === 1 || month === 12) {
    season = `${month === 12 ? year + 1 : year}_S1`;
  ...
  const $ = await fetch(`https://ani.gamer.com.tw/seasonal.php?c=${season}`, { headers: headers })
    .then((res) => res.text())
    .then(cheerio.load);
  
  const items = $(".program-card")
    .map((i, el) => {
      const favouriteButtonJs = $(el).find(".btn-subscribe").attr("onclick");
      const id = /toggleGather\((\d+),.*\)/.exec(favouriteButtonJs)[1];
      ...
      const regionText = $(el)
        ...
        .replace("台灣", "TW")
        .replace("港澳", "HK");
      return { id, titleTranslate: { "zh-Hant": [title] }, regionText };
    })
    .filter((i, el) => el.regionText.includes(region))
    .get();
  return items;
}
