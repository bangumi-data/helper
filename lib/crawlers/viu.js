...
async function saveIdMap(ID_MAP) {
  ...
  await fs.outputJson(outputFile, {'ID_MAP': ID_MAP }, { spaces: 2 });
}
...
exports.getAll = async function getAll() {
  ...
  const json = await fetch(api).then((res) => res.json());
  const items = json.data.series.map((item) => ({ id: item.product_id, lastVid: item.product_id, title: item.name, img: item.series_image_url }));
  ...
  ID_MAP = Object.fromEntries(Object.entries(ID_MAP).reverse().filter((v,i,a)=>a.findIndex(t=>(t[1] === v[1]))===i).reverse());
  // save ID_MAP
