...
async function correctId(vid) {
  ...
  const json = await fetch(api, { headers: { referer: 'https://m.youku.com/' } })
    .then((res) => res.json());
  return json.data.show.encodeid;
}
...
async function getAllBangumi(condition, page = 1, total = 0) {
  ...
  const items = data
    ...
    .map((item) => ({
      title: item.title,
      href: item.videoLink,
      vid: (item.videoLink.match(/id_(.+)\.html/) || [])[1],
      img: item.img,
      info: item.summary,
    }));
  if (data.length) {
...
exports.getAll = async function getAll() {
  ...
  await items.reduce((sequence, item, idx) => sequence.then(async () => {
    ...
    try {
      const id = await correctId(item.vid);
